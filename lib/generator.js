        ...
        return new Promise(async function (resolve, reject) {
            ...
            for (let i = 0; i < keys.length; i++) {
                ...
                if (!fs.existsSync(key)) {
                    delete fileRec[key];
                    if (path.parse(key).base === "infos.json") {
                        infoChanged = true;
                    }
    ...
    renderAll: function () {
        ...
        */
        ...
        return new Promise(async function (resolve) {
            ...
            for (let i = 0; i < allChangedFileList.length; i++) {
                const changed = allChangedFileList[i];
                const infosjson = path.join("source", changed.name, "infos.json");
                const infos = JSON.parse(fs.readFileSync(infosjson, "utf-8"));
                if (infos.type !== "page" && infos.type !== "site") {
                    throw new Error("Unknown type at " + infosjson);
                }
                for (let j = 0; j < changed.files.length; j++) {
                    const filePath = changed.files[j];
                    if ("infos.json" === path.relative(path.join("source", changed.name), filePath)) {
                        ...
                        if (infos.type === "page") {
                            that.renderPage(indexPath, infos.title);
                        } else {
                            that.renderCommonFile(indexPath);
                        } 
